/**
 * Update Customer JavaScript Document.
 * 
 ...
 */
 ...
 (function () {
    ...
    /**
     * Submits the form and attempts to update customer account
     */
    var create = function () {
        document.querySelector('.create-customer-notifications').innerHTML = '';
        document.querySelector('.button-update-customer').setAttribute('disabled', 'disabled');
        document.querySelector('.button-cancel').setAttribute('disabled', 'disabled');
        document.querySelector('.button-update-customer').innerHTML = '<div class="spinner-grow spinner-grow-sm me-3" ></div>' +
            '<span>Updating Customer Account...</span>';
        ...
        API.request({
            path: '/json/customers/' + document.querySelector('[name=slug]').value,
            method: 'put',
            cors: true,
            data: postData,
                ...
                if (code !== 200) {
                    document.querySelector('.create-customer-notifications').innerHTML = '<div class="col-12 my-3">' +
                        '<div class="alert alert-danger">' +
                            ...
                            'Failed to update customer account: ' + data.message +
            ...
            error: function () {
                document.querySelector('.create-customer-notifications').innerHTML = '<div class="col-12 my-3">' +
                    '<div class="alert alert-danger">' +
                        '<span class="fe fe-alert-triangle me-3"></span>' +
                        'Failed to update customer account, no response from server.' +
                    '</div>' +
                '</div>';
            },
            always: function () {
                document.querySelector('.button-update-customer').removeAttribute('disabled');
                document.querySelector('.button-cancel').removeAttribute('disabled', 'disabled');
                document.querySelector('.button-update-customer').innerHTML = 'Update Customer Account';
            },
        });
    };
    ...
    if (document.querySelector('.button-update-customer')) {
        document.querySelector('.button-update-customer').addEventListener('click', create);
    }
