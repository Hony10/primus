...
(function () {
    ...
    // Loads deal data
    var loadDeal = function () {
        API.request({
            path: '/json/deals/' + document.querySelector('[name=loaded-deal-slug]').value,
            method: 'get',
            cors: true,
            ...
            success: function (code, data) {
                ...
                document.querySelector('[name=colour]').value = data.colour;
                document.querySelector('[name=deduction]').value = data.deduction;
                document.querySelector('[name=deduction-ivan]').value = data.deduction_ivan;
                document.querySelectorAll('.agent-percent').forEach(function (element) {
                    var id = parseInt(element.getAttribute('data-agent-id'));
                    var percent = 0;
                    Object.keys(data.assignments).forEach(function (key) {
                        percent = parseInt(key) === id && data.assignments[key] ? data.assignments[key] : percent;
                    });
    ...
    var saveDeal = function () {
        ...
        var postData = {
            ...
            deduction: document.querySelector('[name=deduction]').value ? document.querySelector('[name=deduction]').value : 0,
            deduction_ivan: document.querySelector('[name=deduction-ivan]').value ? document.querySelector('[name=deduction-ivan]').value : 0,
        ...
        API.request({
            ...
            success: function (code, data) {
                if (code !== 200) {
                    return errorMessage('Failed to update deal: ' + data.message);
                }

                location.href = '/deals/';
            },
            error: function () {
                errorMessage('Failed to update deal, server is not responding, please try again.');
            },
            always: function () {
                loading(false);
            },
    ...
    // Loads users and then the deal and populates the form
    populateAgentsList()
    .then(loadDeal);

    document.querySelector('.button-update-deal').addEventListener('click', saveDeal);
